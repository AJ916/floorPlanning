...
class FloorPlan:
    def __init__(self, shape_dimensions):
        """
        shape_dimensions: list of tuples [(width1, height1), (width2, height2), (width3, height3)]
        representing dimensions of the three stacked rectangles
        """
        self.shape_dimensions = shape_dimensions
        self.rooms = []
        ...
        for width, height in shape_dimensions:
            ...
            y_offset += height
        # After computing self.floor_regions
        self.floor_width = max(region['width'] for region in self.floor_regions)
        self.floor_height = sum(region['height'] for region in self.floor_regions)
        ...
        for attempt in range(max_attempts):
            ...
            for room in sorted_rooms:
                ...
                for region in self.floor_regions:
                    ...
                    for _ in range(100):  # Try 100 random positions within this region
    ...
    def visualize(self):
        ...
        max_width = max(region['x'] + region['width'] for region in self.floor_regions)
        max_height = sum(region['height'] for region in self.floor_regions)
...
if __name__ == "__main__":
    # Define floor shape - three stacked rectangles of increasing width
    shape_dimensions = [
    ...
    floor_plan = FloorPlan(shape_dimensions)
    ...
    floor_plan.add_room("secretRoom", 2, 3, max_expansion=0)  # No expansion allowed for secret room
    ...
    floor_plan.add_adjacency("Bedroom 1", "Bedroom 2")
    floor_plan.add_adjacency("Bedroom 2", "Hallway")
    floor_plan.add_adjacency("Hallway", "Bathroom")
    floor_plan.add_adjacency("secretRoom", "Bedroom 2")
